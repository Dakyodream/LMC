...
public class Track extends SpiritOfBDD {
	...
	private ArrayList<Style> mListOfStyle = new ArrayList<Style>(
			Arrays.asList(Style.values()));

	// new ArrayList<MyEnum>(Arrays.asList(MyEnum.values()));

	/**
	 * remplacer les type int et string pour miDuree, msAdrPiste et msAdrImg par
	 * des type plus adapté.
	 */
	...
	public Track(int iKeyOfTrack, String sTracksName, String sTrackAddr,
			FormatOfPiste TrackFormat, ArrayList<Style> listOfStyle, int iLength) {
		super.setKey(iKeyOfTrack);
		super.setName(sTracksName);
		this.setTrackAddr(sTrackAddr);
		this.setTrackFormat(TrackFormat);
